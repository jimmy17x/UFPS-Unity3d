...
public class GuiSurvival : MonoBehaviour {
	...
	*/
	...
	//FPS camera and weapon rendering script 
	public Camera fpsCamera;
	private vp_FPCamera weaponScript;

	//gun accuracy parameter
	public  float ACCURACY_DECREASE_FACTOR = 0.03f;
	private  readonly int GUN_AMPLITUDE_STEP = 1000; // sets x and y value for  Vector3 in shake amplitude
	private float ACCURACY_INCREASE_FACTOR ;
	private float LOWEST_HEALTH_MARGIN  ; //health margin after which gun accuracy starts to change
	private bool isNewAmplitudeSet ;
	private Vector3 normalShakeAmplitude;
	private float normalShakeSpeed;
	public readonly float  MAX_SHAKE_SPEED = .3f;

	private float previousHealth ; //tracks currentHealth ticks to update gun accuracy
	...
	//decreases gun  accuracy by factor 
	void decreaseGunAccuracy ()
	{

		//set increased shake amplitude for succeeding gun shakes
		if (!isNewAmplitudeSet)
		{
			weaponScript.ShakeAmplitude = new Vector3(ACCURACY_DECREASE_FACTOR * GUN_AMPLITUDE_STEP , ACCURACY_DECREASE_FACTOR * GUN_AMPLITUDE_STEP,0);
			isNewAmplitudeSet = !isNewAmplitudeSet;
		}

		//increase gun shake by factor
		if(weaponScript.ShakeSpeed < MAX_SHAKE_SPEED)
			weaponScript.ShakeSpeed += ACCURACY_DECREASE_FACTOR;
			
	}

	//increases gun  accuracy by factor 
	void increaseGunAccuracy ()
	{

		//decrease gun shake by factor
		if(weaponScript.ShakeSpeed > 0.0f)
			weaponScript.ShakeSpeed -= ACCURACY_INCREASE_FACTOR ;
		
	}

	//reset gun accuracy to noraml values

	void resetGunAccuracy()
	{
		weaponScript.ShakeSpeed = normalShakeSpeed;
		weaponScript.ShakeAmplitude = normalShakeAmplitude;
		isNewAmplitudeSet = !isNewAmplitudeSet;
	}
	...
	void Update()
	{
		//when players health improves ,improve gun accuracy
		if (previousHealth < currentHealth) 
		{
			previousHealth = currentHealth;
			increaseGunAccuracy();

			//if health is normal reset gun accuracy to normal
			if(  isNewAmplitudeSet && currentHealth > LOWEST_HEALTH_MARGIN )
			{
				resetGunAccuracy();
			}
		...
		{
			CharacterDeath ();
		} else if ( ( currentHealth < LOWEST_HEALTH_MARGIN ) && ( Mathf.Abs( previousHealth -  currentHealth )  > 1 ) ) 
		{
			decreaseGunAccuracy();
			previousHealth = currentHealth;
		}
